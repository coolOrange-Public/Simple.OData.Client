...
namespace Simple.OData.Client.Tests.Core
{
    ...
    {
        private static ITypeCache _typeCache
        {
        ...
        public void TryConvert(object value, Type sourceType, Type targetType)
        {
            ...
            object targetValue;
            var result = _typeCache.TryConvert(sourceValue, targetType, out targetValue);
            Assert.True(result);
        ...
        public void TryConvertValueWithoutImplicitDateConversionFails(object value, Type sourceType, Type targetType)
        {
            ...
            var result = _typeCache.TryConvert(sourceValue, targetType, out convertedValue);
            Assert.False(result);
        }
        ...
        {
	        Func<object, object> converter = c => new PrimitiveType(new Guid(c.ToString()));
	        _typeCache.Converter.RegisterTypeConverter<PrimitiveType>(converter);
            
            var source = Guid.NewGuid();
            object converted;
            var result = _typeCache.TryConvert(source, typeof(PrimitiveType), out converted);
            Assert.True(result);
