...
namespace Simple.OData.Client
{
	...
	public class FluentCommand
	{
		...
		internal FluentCommand(FluentCommand parent,
			ConcurrentDictionary<object, IDictionary<string, object>> batchEntries)
		{
			Details = new FluentCommandDetails(parent != null ? parent.Details : null, batchEntries);
		}
		...
		public FluentCommand Filter(string filter)
		{
			if (string.IsNullOrEmpty(Details.Filter))
				...
				Details.Filter = string.Format("({0}) and ({1})", Details.Filter, filter);
			return this;
		}
		...
		public FluentCommand QueryOptions(string queryOptions)
		{
			if (Details.QueryOptions == null)
				...
				Details.QueryOptions = string.Format("{0}&{1}", Details.QueryOptions, queryOptions);
			return this;
		}
		...
		{
			Details.EntryData = value.Select(x => new KeyValuePair<string, object>(x.Reference, x.Value))
				.ToIDictionary();
		...
		{
			get { return Details.SelectColumns; }
		}
		...
		{
			return columns.SelectMany(x =>
				x.Key.Split(',').Select(y => new KeyValuePair<string, bool>(y.Trim(), x.Value)));
		}
	}
}
