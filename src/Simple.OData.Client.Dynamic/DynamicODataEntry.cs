...
namespace Simple.OData.Client
{
	public class DynamicODataEntry : ODataEntry, IDynamicMetaObjectProvider
	{
		...
		internal DynamicODataEntry(IDictionary<string, object> entry, ITypeCache typeCache) : base(
			ToDynamicODataEntry(entry, typeCache))
		{
			TypeCache = typeCache;
		}
		...
		private class DynamicEntryMetaObject : DynamicMetaObject
		{
			...
			public override DynamicMetaObject BindConvert(ConvertBinder binder)
			{
				...
				var valueExpression = Expression.Convert(
					Expression.Invoke(convertValueExpression, Expression.Constant(HasValue),
