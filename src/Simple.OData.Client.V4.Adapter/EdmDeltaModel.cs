...
namespace Simple.OData.Client.V4.Adapter
{
	internal class EdmDeltaModel : IEdmModel
	{
		...
		{
			_source = source;
			_entityType = new EdmEntityType(entityType.Namespace, entityType.Name, null, entityType.IsAbstract,
				entityType.IsOpen, entityType.HasStream);
					...
					var navInfo = new EdmNavigationPropertyInfo() {
						...
						Target = property.Partner != null
							...
							: property.Type.TypeKind() == EdmTypeKind.Collection
								? (property.Type.Definition as IEdmCollectionType).ElementType.Definition as
								IEdmEntityType
		...
		{
			return _source.FindDeclaredBoundOperations(bindingType);
		}

		public IEnumerable<IEdmOperation> FindDeclaredBoundOperations(string qualifiedName, IEdmType bindingType)
		{
			return _source.FindDeclaredBoundOperations(qualifiedName, bindingType);
		}

		public IEnumerable<IEdmOperation> FindDeclaredOperations(string qualifiedName)
		{
			return _source.FindDeclaredOperations(qualifiedName);
		}

		public IEdmTerm FindDeclaredTerm(string qualifiedName) { return _source.FindDeclaredTerm(qualifiedName); }

		public IEnumerable<IEdmVocabularyAnnotation>
			FindDeclaredVocabularyAnnotations(IEdmVocabularyAnnotatable element)
		{
			return _source.FindDeclaredVocabularyAnnotations(element);
		}

		public IEnumerable<IEdmStructuredType> FindDirectlyDerivedTypes(IEdmStructuredType baseType)
		{
			return _source.FindDirectlyDerivedTypes(baseType);
		}
		...
		{
			get { return _source.VocabularyAnnotations; }
		}
		...
		{
			get { return _source.DirectValueAnnotationsManager; }
		}
