	...
	public class BatchWriter : BatchWriterBase
	{
			...
			Uri uri, string method, string collection, string contentId, bool resultRequired)
		{
			...
			if (method == RestVerbs.Post || method == RestVerbs.Put || method == RestVerbs.Patch ||
			    method == RestVerbs.Merge)
			{
				message.SetHeader(HttpLiteral.Prefer,
					resultRequired ? HttpLiteral.ReturnRepresentation : HttpLiteral.ReturnMinimal);
			}

			if (collection != null &&
			    _session.Metadata.EntityCollectionRequiresOptimisticConcurrencyCheck(collection) &&
			    (method == RestVerbs.Put || method == RestVerbs.Patch || method == RestVerbs.Merge ||
