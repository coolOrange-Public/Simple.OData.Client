...
namespace Simple.OData.Client.Tests.Core
{
	...
	public class TypedExpressionV4Tests : TypedExpressionTests
	{
		...
		public void FilterEntitiesByStringPropertyWithContains()
		{
			...
			Assert.Equal("(ProductName in ('Chai','Milk','Water'))",
		...
		public void FilterEntitiesByNestedPropertyWithContains()
		{
			...
			Assert.Equal("(Nested/ProductName in ('Chai','Milk','Water'))",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void FilterEntitiesByComplexConditionWithContains()
		{
			...
			Assert.Equal("(tolower(ProductName) in ('chai','milk','water'))",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void FilterEntitiesWithContainsAndNotContains()
		{
			...
			Expression<Func<TestEntity, bool>>
				filter = x => ids.Contains(x.ProductID) && !names.Contains(x.ProductName);
			Assert.Equal("(ProductID in (1,2,3)) and not (ProductName in ('Chai','Milk','Water'))",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
	}
	...
	public abstract class TypedExpressionTests : CoreTestBase
	{
		...
		public void And()
		{
			...
			Assert.Equal("ProductID eq 1 and ProductName eq 'Chai'",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void Or()
		{
			...
			Assert.Equal("ProductName eq 'Chai' or ProductID eq 1",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void Precedence()
		{
			Expression<Func<TestEntity, bool>> filter = x =>
				(x.ProductID == 1 || x.ProductID == 2) && x.ProductName == "Chai";
			Assert.Equal("(ProductID eq 1 or ProductID eq 2) and ProductName eq 'Chai'",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void GreaterOrEqualNumeric()
		{
			...
			Assert.Equal(string.Format("ProductID ge 1.5{0}", FormatSettings.DoubleNumberSuffix),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void EqualLong()
		{
			...
			Assert.Equal(string.Format("ProductID eq 1{0}", FormatSettings.LongNumberSuffix),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void EqualDecimal()
		{
			...
			Assert.Equal(string.Format("Price eq 1{0}", FormatSettings.DecimalNumberSuffix),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void EqualDecimalWithFractionalPart()
		{
			...
			Assert.Equal(string.Format("Price eq 1.23{0}", FormatSettings.DecimalNumberSuffix),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void EqualGuid()
		{
			...
			Assert.Equal(
				string.Format("LinkID eq {0}", FormatSettings.GetGuidFormat("00000000-0000-0000-0000-000000000000")),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void EqualDateTime()
		{
			if (FormatSettings.ODataVersion < 4)
			{
				...
				Assert.Equal("CreationTime eq datetime'2013-01-01T00:00:00'",
					ODataExpression.FromLinqExpression(filter).AsString(_session));
			}
		}
		...
		{
			Expression<Func<TestEntity, bool>> filter = x =>
				x.Updated == new DateTimeOffset(new DateTime(2013, 1, 1, 0, 0, 0, DateTimeKind.Utc));
			Assert.Equal(
				string.Format("Updated eq {0}", FormatSettings.GetDateTimeOffsetFormat("2013-01-01T00:00:00Z")),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void EqualTimeSpan()
		{
			...
			Assert.Equal(string.Format("Period eq {0}'PT1H2M3S'", FormatSettings.TimeSpanPrefix),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void StringToLowerEqual()
		{
			...
			Assert.Equal("tolower(ProductName) eq 'chai'",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void StringToUpperEqual()
		{
			...
			Assert.Equal("toupper(ProductName) eq 'CHAI'",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void StringStartsWithEqual()
		{
			...
			Assert.Equal("startswith(ProductName,'Ch') eq true",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void StringEndsWithEqual()
		{
			...
			Assert.Equal("endswith(ProductName,'Ch') eq true",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void StringContainsEqualTrue()
		{
			...
			Assert.Equal(string.Format("{0} eq true", FormatSettings.GetContainsFormat("ProductName", "ai")),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void StringContainsEqualFalse()
		{
			...
			Assert.Equal(string.Format("{0} eq false", FormatSettings.GetContainsFormat("ProductName", "ai")),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void StringContains()
		{
			...
			Assert.Equal(FormatSettings.GetContainsFormat("ProductName", "ai"),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void StringContainedIn()
		{
			...
			Assert.Equal(FormatSettings.GetContainedInFormat("ProductName", "Chai"),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		{
			Expression<Func<TestEntity, bool>> filter = x => !x.ProductName.Contains("ai");
			Assert.Equal(string.Format("not {0}", FormatSettings.GetContainsFormat("ProductName", "ai")),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void StringToLowerAndContains()
		{
			...
			Assert.Equal(FormatSettings.GetContainsFormat("tolower(ProductName)", "Chai"),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void IndexOfStringEqual()
		{
			...
			Assert.Equal("indexof(ProductName,'ai') eq 1",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void SubstringWithPositionEqual()
		{
			...
			Assert.Equal("substring(ProductName,1) eq 'hai'",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void ReplaceStringEqual()
		{
			...
			Assert.Equal("replace(ProductName,'a','o') eq 'Choi'",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void ConcatEqual()
		{
			...
			Assert.Equal("concat(ProductName,'Chai') eq 'ChaiChai'",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void RoundEqual()
		{
			...
			Assert.Equal(string.Format("round(Price) eq 1{0}", FormatSettings.DecimalNumberSuffix),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void FloorEqual()
		{
			...
			Assert.Equal(string.Format("floor(Price) eq 1{0}", FormatSettings.DecimalNumberSuffix),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void CeilingEqual()
		{
			...
			Assert.Equal(string.Format("ceiling(Price) eq 2{0}", FormatSettings.DecimalNumberSuffix),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void EqualNestedPropertyLengthOfStringEqual()
		{
			...
			Assert.Equal("length(Nested/ProductName) eq 4",
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void FilterWithEnum()
		{
			...
			Assert.Equal(
				string.Format("Address/Type eq {0}",
					FormatSettings.GetEnumFormat(AddressType.Corporate, typeof(AddressType), "NorthwindModel")),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		public void FilterWithEnum_PrefixFree()
		{
			...
			{
				Expression<Func<TestEntity, bool>> filter = x => x.Address.Type == AddressType.Corporate;
				Assert.Equal(string.Format("Address/Type eq {0}", FormatSettings.GetEnumFormat(AddressType.Corporate,
						typeof(AddressType), "NorthwindModel",
						true)),
					ODataExpression.FromLinqExpression(filter).AsString(_session));
			}
			finally
		...
		public void FilterWithEnum_HasFlag()
		{
			...
			Assert.Equal(
				string.Format("Address/Type has {0}",
					FormatSettings.GetEnumFormat(AddressType.Corporate, typeof(AddressType), "NorthwindModel")),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		}
		...
		{
			Expression<Func<TestEntity, bool>> filter = x =>
				x.Address.Type.ToString() == AddressType.Corporate.ToString();
			Assert.Equal(
				string.Format("Address/Type eq {0}",
					FormatSettings.GetEnumFormat(AddressType.Corporate, typeof(AddressType), "NorthwindModel")),
				ODataExpression.FromLinqExpression(filter).AsString(_session));
		...
		public void FilterDateTimeRange()
		{
			...
			{
				Assert.Equal(
					"CreationTime ge datetime'2013-01-01T00:00:00Z' and CreationTime lt datetime'2014-02-02T00:00:00Z'",
					ODataExpression.FromLinqExpression(filter).AsString(_session));
			}
			else
			{
		...
		public void ExpressionBuilderGrouping()
		{
			...
			Assert.Equal("(ProductName eq 'Chai' or ProductID eq 1) and (ProductName eq 'Kaffe' or ProductID eq 2)",
		...
		public void FilterEqualityToMappedPropertyOfOtherEntity()
		{
			...
			Expression<Func<TestEntity, bool>> filter = x =>
				x.MappedNameUsingDataMemberAttribute == otherEntity.MappedNameUsingDataMemberAttribute;
