...
namespace Simple.OData.Client.Tests.Core
{
	public class AdapterTests : TestBase
	{
		private class CustomBatchWriter : BatchWriter
		{
			...
			public CustomBatchWriter(ISession session, IDictionary<object, IDictionary<string, object>> batchEntries) :
				base(session, batchEntries)
			{
				this.session = session;
			}

			protected async override Task<object> CreateOperationMessageAsync(Uri uri, string method, string collection,
				string contentId, bool resultRequired)
			{
				var result = await base.CreateOperationMessageAsync(AppendToken(uri), method, collection, contentId,
					resultRequired);
				var request = result as IODataRequestMessage;
				if (request != null)
				{
		...
		{
			public override Func<ISession, IODataAdapter> CreateAdapterLoader(string metadataString,
