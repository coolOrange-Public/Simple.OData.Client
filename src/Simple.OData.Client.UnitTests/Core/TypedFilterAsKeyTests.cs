...
namespace Simple.OData.Client.Tests.Core
{
	...
	public abstract class TypedFilterAsKeyTests : CoreTestBase
	{
		...
		public async Task FindAllByFilterTwoClausesWithOr()
		{
			...
			var commandText = await command.GetCommandTextAsync();
			Assert.Equal(
				"Products?$filter=%28ProductID%20ne%201%20or%20ProductID%20ne%202%29%20and%20ProductID%20ne%203",
				commandText);
		}
		...
		public async Task FindAllByFilterAsNotKeyEqual()
		{
			...
			var commandText = await command.GetCommandTextAsync();
			Assert.Equal(
				string.Format("Products?$filter=not%20{0}ProductID%20eq%201{1}", Uri.EscapeDataString("("),
		...
		public async Task FindAllByFilterWithDateTimeOffset()
		{
			...
			var commandText = await command.GetCommandTextAsync();
			Assert.Equal(
				string.Format("Orders?$filter=ShippedDateTimeOffset%20gt%20{0}",
		...
		public async Task FindByGuidFilterEqual()
		{
			...
			var commandText = await command.GetCommandTextAsync();
			Assert.Equal(
				string.Format("TypeWithGuidKey({0})",
					Uri.EscapeDataString(FormatSettings.GetGuidFormat(key.ToString()))), commandText);
		}
