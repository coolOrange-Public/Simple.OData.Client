	...
	public class FindDynamicTests : TestBase
	{
		...
		public async Task ExpandSecondLevel()
		{
			...
			Assert.Equal(ExpectedCountOfCondimentsProducts,
				(product.Category.Products as IEnumerable<dynamic>).Count());
		}
		...
		public async Task ExpandMultipleLevelsWithCollection()
		{
			...
			Assert.Equal("Condiments",
				(product.Category.Products as IEnumerable<dynamic>).First().Category.CategoryName);
		}
		...
		public async Task AllDerivedClassEntriesWithAnnotations()
		{
			...
			Assert.Equal("NorthwindModel.Ship",
				((transport as IEnumerable<dynamic>).Single()[
					FluentCommand.AnnotationsLiteral] as ODataEntryAnnotations).TypeName);
		}
