...
namespace Simple.OData.Client.Tests.FluentApi
{
	...
	public class BatchTests : TestBase
	{
		...
		public async Task Success()
		{
			...
			batch += c =>
				c.InsertEntryAsync("Products", new Entry() { { "ProductName", "Test1" }, { "UnitPrice", 10m } }, false);
			batch += c =>
				c.InsertEntryAsync("Products", new Entry() { { "ProductName", "Test2" }, { "UnitPrice", 20m } }, false);
		...
		public async Task NestedBatch()
		{
			...
			batch2 += c =>
				c.InsertEntryAsync("Products", new Entry() { { "ProductName", "Test3" }, { "UnitPrice", 30m } }, false);
			batch2 += c =>
				c.InsertEntryAsync("Products", new Entry() { { "ProductName", "Test4" }, { "UnitPrice", 40m } }, false);
		...
		public async Task SuccessWithResults()
		{
			...
			batch += async x =>
			{
				product1 = await x.InsertEntryAsync("Products",
					new Entry() { { "ProductName", "Test1" }, { "UnitPrice", 10m } });
			};
			batch += async x =>
			{
				product2 = await x.InsertEntryAsync("Products",
					new Entry() { { "ProductName", "Test2" }, { "UnitPrice", 20m } });
			};
			await batch.ExecuteAsync();
		...
		public async Task PartialFailures()
		{
			...
			batch += c => c.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test2" }, { "UnitPrice", 10m }, { "SupplierID", 0xFFFF } }, false);
		...
		public async Task MultipleUpdateEntrySingleBatch()
		{
			...
			var batch = new ODataBatch(settings);
			batch += async c => product = await c.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test11" }, { "UnitPrice", 21m } });
			await batch.ExecuteAsync();
		...
		public async Task MultipleUpdatesEntriesSingleBatch()
		{
			...
			var batch = new ODataBatch(settings);
			batch += async c => product = await c.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test11" }, { "UnitPrice", 121m } });
			batch += async c => product = await c.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test12" }, { "UnitPrice", 121m } });
			batch += async c => product = await c.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test13" }, { "UnitPrice", 121m } });
			await batch.ExecuteAsync();

			batch = new ODataBatch(settings);
			batch += c =>
				c.For("Products").Filter("UnitPrice eq 121").Set(new Entry() { { "UnitPrice", 122m } })
					.UpdateEntriesAsync();
			await batch.ExecuteAsync();
		...
		public async Task UpdateDeleteSingleBatch()
		{
			...
			var batch = new ODataBatch(settings);
			batch += async x => product = await x.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test11" }, { "UnitPrice", 21m } });
			await batch.ExecuteAsync();
		...
		public async Task InsertUpdateDeleteSeparateBatches()
		{
			...
			var batch = new ODataBatch(settings);
			batch += c =>
				c.InsertEntryAsync("Products", new Entry() { { "ProductName", "Test12" }, { "UnitPrice", 21m } },
					false);
			await batch.ExecuteAsync();
		...
		public async Task InsertSingleEntityWithSingleAssociationSingleBatch()
		{
			...
			var batch = new ODataBatch(settings);
			batch += async x =>
				category = await x.InsertEntryAsync("Categories", new Entry() { { "CategoryName", "Test13" } });
			batch += c => c.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test14" }, { "UnitPrice", 21m }, { "Category", category } }, false);
			await batch.ExecuteAsync();
		...
		public async Task InsertSingleEntityWithMultipleAssociationsSingleBatch()
		{
			...
			IDictionary<string, object> product2 = null;
			var batch = new ODataBatch(settings);
			batch += async c => product1 = await c.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test15" }, { "UnitPrice", 21m } });
			batch += async c => product2 = await c.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test16" }, { "UnitPrice", 22m } });
			batch += c => c.InsertEntryAsync("Categories",
				new Entry() { { "CategoryName", "Test17" }, { "Products", new[] { product1, product2 } } }, false);
			await batch.ExecuteAsync();
		...
		public async Task DeleteEntriesExisting()
		{
			...
			var batch = new ODataBatch(settings);
			batch += async c =>
				await c.InsertEntryAsync("Products", new Entry() { { "ProductName", "Test19" }, { "UnitPrice", 21m } });
			await batch.ExecuteAsync();
		...
		public async Task DeleteEntriesMultiple()
		{
			...
			var batch = new ODataBatch(settings);
			batch += async c =>
				await c.InsertEntryAsync("Products",
					new Entry() { { "ProductName", "Test21" }, { "UnitPrice", 111m } });
			batch += async c =>
				await c.InsertEntryAsync("Products",
					new Entry() { { "ProductName", "Test22" }, { "UnitPrice", 111m } });
			batch += async c =>
				await c.InsertEntryAsync("Products",
					new Entry() { { "ProductName", "Test23" }, { "UnitPrice", 111m } });
			await batch.ExecuteAsync();
		...
		public async Task DeleteEntriesNonExistingThenInsert()
		{
			...
			batch += async c => product = await c.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test15" }, { "UnitPrice", 21m } });
			batch += c => c.InsertEntryAsync("Categories",
				new Entry() { { "CategoryName", "Test17" }, { "Products", new[] { product } } }, false);
			await batch.ExecuteAsync();
		}
		...
		public async Task UpdateWithResultRequired()
		{
			...
			var batch = new ODataBatch(settings);
			batch += async x => product = await x.InsertEntryAsync("Products",
				new Entry() { { "ProductName", "Test11" }, { "UnitPrice", 21m } }, true);
			await batch.ExecuteAsync();

			batch = new ODataBatch(settings);
			batch += async c =>
				product1 = await c.UpdateEntryAsync("Products", product, new Entry() { { "UnitPrice", 22m } }, true);
			await batch.ExecuteAsync();
		...
		public async Task Function()
		{
			...
			var batch = new ODataBatch(settings);
			var result = 0;
			batch += async c => result = await c.Unbound().Function("ParseInt").Set(new Entry() { { "number", "1" } })
		...
		public async Task WithHttpHeaders()
		{
			...
			var settings = CreateDefaultSettings().WithHttpMock();
			settings.BeforeRequest = r => headers = r.Headers.ToDictionary(x => x.Key, x => x.Value.First());
			...
			string value;
			Assert.True(headers.TryGetValue("batchHeader", out value) && value == "batchHeaderValue");
		}
	}
}