...
namespace Simple.OData.Client.Tests.BasicApi
{
	...
	public class ClientReadOnlyTests : TestBase
	{
		...
		{
			var client = new ODataClient(CreateDefaultSettings().WithHttpMock());
			var orderDetail = await client
				.GetEntryAsync("Order_Details", new Entry() { { "OrderID", 10248 }, { "ProductID", 11 } })
				.ConfigureAwait(false);
		...
		public async Task GetEntryNonExisting()
		{
			...
			await AssertThrowsAsync<WebRequestException>(async () =>
					await client.GetEntryAsync("Products", new Entry() { { "ProductID", -1 } }).ConfigureAwait(false))
				.ConfigureAwait(false);
		}
		...
		public async Task ExecuteScalarFunctionWithLongParameter()
		{
			...
			var result = await client
				.ExecuteFunctionAsScalarAsync<long>("PassThroughLong", new Entry() { { "number", 1L } })
				.ConfigureAwait(false);
		...
		public async Task ExecuteScalarFunctionWithDateTimeParameter()
		{
			...
			var result = await client
				.ExecuteFunctionAsScalarAsync<DateTime>("PassThroughDateTime", new Entry() { { "dateTime", dateTime } })
				.ConfigureAwait(false);
		...
		public async Task ExecuteScalarFunctionWithGuidParameter()
		{
			...
			var result = await client
				.ExecuteFunctionAsScalarAsync<Guid>("PassThroughGuid", new Entry() { { "guid", guid } })
				.ConfigureAwait(false);
