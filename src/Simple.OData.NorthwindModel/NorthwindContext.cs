...
namespace NorthwindModel
{
	public class NorthwindContext : DbContext
	{
		...
		protected override void OnModelCreating(DbModelBuilder modelBuilder)
		{
			...
			modelBuilder.Entity<Category>().Property(x => x.CategoryID)
				.HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
			modelBuilder.Entity<Customer>().Property(x => x.CustomerID)
				.HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
			modelBuilder.Entity<Employee>().Property(x => x.EmployeeID)
				.HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
			modelBuilder.Entity<Order>().Property(x => x.OrderID)
				.HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
			modelBuilder.Entity<Product>().Property(x => x.ProductID)
				.HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
			modelBuilder.Entity<Shipper>().Property(x => x.ShipperID)
				.HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
			modelBuilder.Entity<Transport>().Property(x => x.TransportID)
				.HasDatabaseGeneratedOption(DatabaseGeneratedOption.None);
		...
		public override int SaveChanges()
		{
			...
			{
				SetKey<Category>(entry, x =>
				{
					if (x.CategoryID == 0) { x.CategoryID = ++NextId; }
				});
				SetKey<Employee>(entry, x =>
				{
					if (x.EmployeeID == 0) { x.EmployeeID = ++NextId; }
				});
				SetKey<Order>(entry, x =>
				{
					if (x.OrderID == 0) { x.OrderID = ++NextId; }
				});
				SetKey<Product>(entry, x =>
				{
					if (x.ProductID == 0) { x.ProductID = ++NextId; }
				});
				SetKey<Shipper>(entry, x =>
				{
					if (x.ShipperID == 0) { x.ShipperID = ++NextId; }
				});
				SetKey<Transport>(entry, x =>
				{
					if (x.TransportID == 0) { x.TransportID = ++NextId; }
				});
			}
