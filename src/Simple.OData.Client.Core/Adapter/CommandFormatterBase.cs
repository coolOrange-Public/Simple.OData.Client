	...
	public abstract class CommandFormatterBase : ICommandFormatter
	{
		...
		private string FormatClauses(ResolvedCommand command, IList<string> queryClauses = null)
		{
			...
			if (command.Details.Filter != null)
				queryClauses.Add(string.Format("{0}={1}", ODataLiteral.Filter,
					EscapeUnescapedString(command.Details.Filter)));

			if (command.Details.Search != null)
				queryClauses.Add(string.Format("{0}={1}", ODataLiteral.Search,
					EscapeUnescapedString(command.Details.Search)));
			...
			if (command.Details.QueryOptionsKeyValues != null)
			{
				...
				{
					queryClauses.Add(string.Format("{0}={1}", kv.Key,
						ODataExpression.FromValue(kv.Value).Format(new ExpressionContext(_session))));
				}
			}
		...
		protected string FormatOrderByItem(KeyValuePair<string, bool> pathWithOrder, EntityCollection entityCollection)
		{
			...
			else
			{
				...
				{
					throw new UnresolvableObjectException(items[0],
						string.Format("Property path [{0}] not found", items[0]));
				}
			}
		}
