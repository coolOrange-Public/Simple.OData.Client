...
namespace Simple.OData.Client
{
	public abstract class RequestWriterBase : IRequestWriter
	{
		...
		{
			get { return TypeCache.Converter; }
		}
		...
		public async Task<ODataRequest> CreateInsertRequestAsync(string collection, string commandText,
			IDictionary<string, object> entryData, bool resultRequired, bool deep,
			IDictionary<string, string> headers = null)
		{
		...
		public async Task<ODataRequest> CreateUpdateRequestAsync(string collection, string entryIdent,
			IDictionary<string, object> entryKey, IDictionary<string, object> entryData, bool resultRequired,
			IDictionary<string, string> headers = null)
		{
...
#pragma warning disable CS0618 // Type or member is obsolete - purpose of this test
			updateMethod = _session.Settings.PreferredUpdateMethod == ODataUpdateMethod.Merge
				? RestVerbs.Merge
				: updateMethod;
#pragma warning restore CS0618 // Type or member is obsolete
			...
			var request =
				new ODataRequest(updateMethod, _session, entryIdent, entryData, entryContent, headers: headers) {
					ResultRequired = resultRequired,
		...
		public async Task<ODataRequest> CreateLinkRequestAsync(string collection, string linkName, string entryIdent,
			string linkIdent, IDictionary<string, string> headers = null)
		{
		...
		public async Task<ODataRequest> CreateActionRequestAsync(string commandText, string actionName,
			string boundTypeName, IDictionary<string, object> parameters, bool resultRequired,
			IDictionary<string, string> headers = null)
		{
		...
		public async Task<ODataRequest> CreatePutRequestAsync(string commandText, Stream stream,
			string mediaType = null, bool optimisticConcurrency = false, IDictionary<string, string> headers = null)
		{
			...
			IDictionary<string, object> entryData, Stream stream, bool resultRequired, string mediaType = null)
		{
			...
			AssignHeaders(request);
			var slugHeader = WriteEntrySlugHeader(collection, entryData);
			request.Headers.Add(HttpLiteral.Slug, slugHeader.ToString());
			return request;
		}


		protected abstract Task<Stream> WriteEntryContentAsync(string method, string collections, string commandText,
			IDictionary<string, object> entryData, bool resultRequired, bool deep = false);
		...
		protected abstract Task<Stream> WriteActionContentAsync(string method, string commandText, string actionName,
			string boundTypeName, IDictionary<string, object> parameters);

		protected abstract Task<Stream> WriteStreamContentAsync(Stream stream, bool writeAsText);

		protected abstract string FormatLinkPath(string entryIdent, string navigationPropertyName,
		...
		private bool HasUpdatedKeyProperties(string collection, IDictionary<string, object> entryKey,
