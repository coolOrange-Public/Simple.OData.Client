			...
			switch (node.Method.Name)
			{
				...
				case "ThenBy":
				{
					if ((node.Arguments[0] is MethodCallExpression
						    ? ((MethodCallExpression)node.Arguments[0]).Method.Name
						    : null) == "Select")
					{
						throw Utils.NotSupportedExpression(node);
					}
				...
				case "Where":
				{
					var filterExpression =
						ODataExpression.FromLinqExpression(node.Arguments[1] is LambdaExpression
							? ((LambdaExpression)node.Arguments[1]).Body
							: null);
