...
namespace Simple.OData.Client
{
	...
	{
		public static bool TryGetOperatorMapping(ODataExpression functionCaller, ExpressionFunction function,
			AdapterVersion adapterVersion,
			out FunctionToOperatorMapping operatorMapping)
		{
			operatorMapping = DefinedMappings.SingleOrDefault(x =>
				x.CanMap(function.FunctionName, function.Arguments.Count, functionCaller, adapterVersion));
			return operatorMapping != null;
		}

		public abstract string Format(ExpressionContext context, ODataExpression functionCaller,
			List<ODataExpression> functionArguments);

		protected abstract bool CanMap(string functionName, int argumentCount, ODataExpression functionCaller,
			AdapterVersion adapterVersion = AdapterVersion.Any);
	...
	{
		public override string Format(ExpressionContext context, ODataExpression functionCaller,
			List<ODataExpression> functionArguments)
		{
			...
			foreach (var item in list)
			{
				...
				listAsString.Append(context.Session.Adapter.GetCommandFormatter()
					.ConvertValueToUriLiteral(item, false));
				delimiter = ",";
			}
			...
			return string.Format("({0} in {1})", functionArguments[0].Format(context), listAsString);
		}

		protected override bool CanMap(string functionName, int argumentCount, ODataExpression functionCaller,
			AdapterVersion adapterVersion = AdapterVersion.Any)
		{
