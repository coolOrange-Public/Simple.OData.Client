...
namespace Simple.OData.Client
{
	...
	public class ODataEntryAnnotations
	{
		...
		internal void Merge(ODataEntryAnnotations src)
		{
			...
			{
				Id = Id ?? src.Id;
				TypeName = TypeName ?? src.TypeName;
				ReadLink = ReadLink ?? src.ReadLink;
				EditLink = EditLink ?? src.EditLink;
				ETag = ETag ?? src.ETag;
				AssociationLinks = AssociationLinks ?? src.AssociationLinks;
				MediaResource = MediaResource ?? src.MediaResource;
				InstanceAnnotations = InstanceAnnotations ?? src.InstanceAnnotations;
			}
		}
	}
}