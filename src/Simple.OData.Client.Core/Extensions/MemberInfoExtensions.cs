...
namespace Simple.OData.Client.Extensions
{
	...
	{
		private static readonly ConcurrentDictionary<MemberInfo, MappingInfo> cache =
			new ConcurrentDictionary<MemberInfo, MappingInfo>();
		...
		private static string GetMappedName(this string name, IList<Attribute> attributes)
		{
			...
			var mappingAttribute =
				attributes.FirstOrDefault(x => supportedAttributeNames.Any(y => x.GetType().Name == y));
