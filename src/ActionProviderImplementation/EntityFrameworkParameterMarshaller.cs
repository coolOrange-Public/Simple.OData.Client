...
namespace ActionProviderImplementation
{
	public class EntityFrameworkParameterMarshaller : IParameterMarshaller
	{
		...
		public object[] Marshall(DataServiceOperationContext operationContext, ServiceAction action,
			object[] parameters)
		{
			var pvalues = action.Parameters.Zip(parameters,
				(parameter, parameterValue) => new { Parameter = parameter, Value = parameterValue });
			var marshalled = pvalues
				.Select(pvalue => GetMarshalledParameter(operationContext, pvalue.Parameter, pvalue.Value)).ToArray();

			return marshalled;
		}

		private object GetMarshalledParameter(DataServiceOperationContext operationContext,
			ServiceActionParameter serviceActionParameter, object value)
		{
			...
			if (parameterKind == ResourceTypeKind.EntityType)
			{
				...
				var updateProvider =
					operationContext.GetService(typeof(IDataServiceUpdateProvider2)) as IDataServiceUpdateProvider2;
				value = updateProvider.GetResource(value as IQueryable,
					serviceActionParameter.ParameterType.InstanceType.FullName);
			...
			else if (parameterKind == ResourceTypeKind.Collection)
			{
				...
				var elementType = (serviceActionParameter.ParameterType as CollectionResourceType).ItemType
					.InstanceType;
