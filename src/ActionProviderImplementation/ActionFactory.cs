...
namespace ActionProviderImplementation
{
	public class ActionFactory
	{
		...
		private ResourceType GetParameterResourceType(Type type)
		{
			...
			if (resourceType.ResourceTypeKind == ResourceTypeKind.EntityType)
			{
				throw new Exception(string.Format("Entity Types ({0}) MUST not be used as non-binding parameters.",
					type.FullName));
			}
			else if (resourceType.ResourceTypeKind == ResourceTypeKind.EntityCollection)
			{
				throw new Exception(string.Format(
					"Entity Type Collections ({0}) MUST not be used as non-binding parameters.", type.FullName));
			}
