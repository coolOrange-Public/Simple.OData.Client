...
namespace ActionProviderImplementation
{
	...
	{
		public ActionInfo(MethodInfo actionMethod, OperationParameterBindingKind bindable,
			string availabilityMethodName)
		{
		...
		public bool IsAvailable(object context, object entity, bool inFeedContext)
		{
			...
			{
				return
					true; // TODO: need a way to verify we are NOT being bound... although I think this is impossible.
			}
		...
		private MethodInfo GetAvailabilityMethod(string availabilityMethodName)
		{
			...
			{
				throw new Exception(
					"If the action is conditionally available you need to provide a method to calculate availability.");
			}
			...
			if (method == null)
				throw new Exception(string.Format("Availability Method {0} was not found on type {1}",
					availabilityMethodName, declaringType.FullName));

			if (method.ReturnType != typeof(bool))
				throw new Exception(string.Format("AvailabilityCheck method ({0}) MUST return bool.",
			...
			if (methodParameters.Count() != 1 || methodParameters.First().ParameterType != actionBindingParameterType)
				throw new Exception(string.Format(
					"AvailabilityCheck method was expected to have this signature 'bool {0}({1})'",
					availabilityMethodName, actionBindingParameterType.FullName));
