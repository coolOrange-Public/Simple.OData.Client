...
namespace ActionProviderImplementation
{
	public class ActionProvider : IDataServiceActionProvider
	{
		private static readonly Dictionary<Type, List<ServiceAction>> _cache =
			new Dictionary<Type, List<ServiceAction>>();

		private static readonly Dictionary<string, ServiceAction> _actionsByName =
			new Dictionary<string, ServiceAction>();
		...
		public bool AdvertiseServiceAction(DataServiceOperationContext operationContext, ServiceAction serviceAction,
			object resourceInstance, bool inFeed, ref Microsoft.Data.OData.ODataAction actionToSerialize)
		{
			var customState = serviceAction.CustomState as ActionInfo;
			return customState.IsAvailable(_context, resourceInstance, inFeed);
		}

		public IDataServiceInvokable CreateInvokable(DataServiceOperationContext operationContext,
			ServiceAction serviceAction, object[] parameters)
		{
		...
		public IEnumerable<ServiceAction> GetServiceActionsByBindingParameterType(
			DataServiceOperationContext operationContext, ResourceType resourceType)
		{
			return GetActions(operationContext).Where(a =>
				a.Parameters.Count > 0 && a.Parameters.First().ParameterType == resourceType);
		}
